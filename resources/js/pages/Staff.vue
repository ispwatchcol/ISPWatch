      ...
      <div class="rounded-2xl bg-white p-6 shadow dark:bg-gray-800">
        ...
        <div v-if="!loading" class="border border-gray-100 dark:border-gray-700 rounded-xl overflow-hidden">
          ...
          <div class="md:hidden divide-y divide-gray-200 dark:divide-gray-700">
            <div v-for="member in sortedStaff" :key="member.id" class="p-4 bg-white dark:bg-gray-800">
              <div class="flex justify-between items-start mb-3">
                <h3 class="font-semibold text-gray-800 dark:text-gray-100 text-sm leading-snug">{{ buildMemberName(member) }}</h3>
                <span
                  class="inline-block rounded-full px-2.5 py-1 text-[10px] font-semibold"
                  :class="getRoleColor(member.role_name)"
                >
                  {{ member.role_name }}
                </span>
              ...
              <div class="grid grid-cols-2 gap-2 text-xs mb-4">
                <div class="col-span-2"><span class="text-gray-500 dark:text-gray-400">Usuario:</span> <span class="text-gray-800 dark:text-gray-200 ml-1">{{ member.email_tenant }}</span></div>
                <div><span class="text-gray-500 dark:text-gray-400">Creado:</span> <span class="text-gray-800 dark:text-gray-200 ml-1">{{ formatDate(member.create_at) }}</span></div>
                <div><span class="text-gray-500 dark:text-gray-400">Acceso:</span> <span class="text-gray-800 dark:text-gray-200 ml-1">{{ member.last_access ? formatDate(member.last_access) : '-' }}</span></div>
              </div>
          ...
          <div v-if="sortedStaff.length === 0" class="py-8 text-center text-gray-500 dark:text-gray-400">
