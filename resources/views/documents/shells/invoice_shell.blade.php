{{--
    Shell fijo para la factura cuando el tenant tiene una plantilla activa en
    document_templates. $body ya llega resuelto (placeholders sustituidos) y
    saneado (App\Services\Templates\TemplateSanitizer) — se imprime tal cual,
    nunca se vuelve a interpretar como Blade/PHP.

    La tabla de ítems y los totales son estructurales y no son editables en
    V1; $body se agrega como bloque adicional (notas/legales propios del
    tenant) antes del pie de página.
--}}
@php
    $statusMap = [
        'draft'     => 'BORRADOR',
    ...
    $brandColor  = $tenant->brand_color ?: '#003087';
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .sbar       { background: {{ $brandColor }}; opacity: 0.9; }
        ...
        .box-hd     { font-size: 8px; font-weight: bold; color: {{ $brandColor }}; text-transform: uppercase;
        ...
        .it-hd th   { background: {{ $brandColor }}; color: #fff; padding: 7px 8px; font-size: 9px; font-weight: bold; }
        ...
        .tot-grand  { background: {{ $brandColor }}; color: #ffffff; }
        .tot-grand td { padding: 7px 10px; font-size: 11px; font-weight: bold; }
        .tot-bal    { background: #e6f0ff; }
        .tot-bal td { font-weight: bold; color: {{ $brandColor }}; }
        ...
        .custom-wrap  { padding: 0 14px 10px; }
        .custom-block { border: 1px solid #e5e7eb; padding: 8px 10px; font-size: 9px; color: #333; line-height: 1.6; }
        .custom-block p { margin-bottom: 4px; }

        .footer { border-top: 2px solid {{ $brandColor }}; padding: 6px 14px 0;
            ...
            <td style="vertical-align:top; padding-right:10px;">
                @if($invoice->notes)
                    <div style="border:1px solid #c5d9f0;padding:8px 10px;background:#fffef0;font-size:9px;color:#555;max-width:300px;">
                        <strong style="color:{{ $brandColor }};">Observaciones:</strong><br>{{ $invoice->notes }}
                    </div>
                @endif
...
{{-- ══ BLOQUE PERSONALIZADO DEL TENANT ══ --}}
@if(trim($body) !== '')
