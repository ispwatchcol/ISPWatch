...
Detectado el 2026-09-22 al documentar el modo para el Centro de Ayuda (§ 70 de la bitácora).
...
> correcto es lo **ganado** (`earned − reversed`), y así está ya el comando (§ 73).
>
...
### ✅ P-43 · Borrar un cliente destruía su histórico de facturación — RESUELTA
...
**✅ RESUELTA el 2026-09-09.**

1. **La decisión de negocio se tomó: se conserva**, con nombre y documento. En Colombia los
   papeles de comercio se conservan diez años (Código de Comercio, arts. 28 y 60) y un cierre
   contable no puede depender de que nadie haya dado de baja a un abonado.
2. Las claves foráneas pasaron a `SET NULL`. **Cinco, no tres**: al inventariarlas aparecieron
...
3. `invoices` y `payments` ganaron `customer_name` y `customer_document`, congelados **al
   emitir** por el trait `FreezesCustomerSnapshot` — no al borrar, porque un documento contable
   dice a quién se le facturó *entonces*. El snapshot va sólo en esas dos: las demás tablas
   cuelgan de una factura o un pago, y repetir el dato personal en cinco sitios no habría dado
   ni un informe nuevo.
4. Los informes sí toleran el nulo, con dos arreglos: las exportaciones y el panel caen a
   `customerDisplayName()`, y `send-reminder` devolvía **500** en vez de su 404 porque leía el
   perfil antes de comprobar el nulo — un fallo latente que este cambio volvía alcanzable.

**Lo que se acepta a conciencia:** el filtro por cliente de los listados no encuentra las
facturas de un cliente dado de baja (busca sobre el cliente vivo). Se llega a ellas por
período, número o rango de fechas. Darles filtro propio exigiría buscar sobre el snapshot, y
no hay caso de uso que hoy lo pida.
