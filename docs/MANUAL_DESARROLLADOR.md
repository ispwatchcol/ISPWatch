...
| Clave | Descripción |
|---|---|
| `HEALTH_CHECK_TOKEN` | Opcional. Si se define, `/health` lo exige en `X-Health-Token` o `?token=`; sin él responde 404. Sin definir, el endpoint queda abierto: no revela secretos |
...
| Comando | Descripción |
|---|---|
| `system:heartbeat` | Deja constancia de que el planificador está vivo. Agendado **cada minuto**; lo lee `/health` para alertar **por silencio**. No lo llames a mano salvo para probar: su valor está en que lo ejecute el planificador |
...
```bash
curl -s https://ispwatch-crm.app/health | python -m json.tool
```
...
| 8a | **El `l2tp-client` NUNCA debe usar el perfil `default`** | En un router que también es servidor PPPoE, `default` trae `local-address` y el túnel se queda con esa IP en vez de la del overlay: ISPWatch pierde el equipo entero sin ningún error visible. El script crea `ISPWatch-VPN` (sin direcciones) y lo recrea **después** de quitar la interfaz — con la interfaz puesta, el perfil está en uso y el `remove` falla |
| 8b | **Que el túnel esté arriba no significa que el router responda** | La sesión puede figurar en `/ppp active`, con contadores subiendo, mientras el equipo del otro lado **no se quedó con la dirección** del overlay y reenvía nuestros paquetes a su gateway. Compruébalo con `php artisan router:probe-overlay <id>` antes de auditar credenciales o firewalls |
| 8c | **`firmware_version` tiene tres formatos** | `7.23.1 (stable)`, la etiqueta `v6`/`v7` que escribe el formulario, y el id de `script_version` (`2`=v7, `3`=v6). Léelo **siempre** con `App\Support\RouterOsVersion`; una expresión `N.N` a pelo da false para `v7` y degrada el transporte a L2TP sin avisar |
...
| 50 | **El primer rol que crea un test se lleva el `id = 1` y con él el bypass de superadmin** | `CheckPermission` salta toda comprobación cuando `role_id == 1`. Con `RefreshDatabase` la tabla arranca vacía, así que el «usuario sin permiso» que fabrica el helper de un test **es** el superadmin: la prueba que debía verificar que el permiso cierra la puerta verifica que el bypass la abre. Y no hay señal — una prueba de permisos que pasa por el bypass **pasa**; sólo se delata la que espera un 403. Pasó en `TenantSelfServiceApiKeyTest`, donde las trece pruebas corrían con los permisos desactivados. Si un test toca permisos, reserva el id 1 en `setUp()` con un rol de plataforma que nadie use, para que los roles de tenant nazcan del 2 en adelante. Ver `BITACORA_TECNICA.md` § 39.2 |
...
| 59 | **El frontend está en `resources/js/pages`** | No en `resources/js/views` ni en `src/`, que no existen. Buscar ahí y concluir que «el frontend vive en otro repositorio» ya costó que una funcionalidad entera saliera sin su mitad visible |
---
...
| **La VPN figura activa y el router no responde a NADA (ni ping, ni API, ni SSH)** | El equipo **no se quedó con la dirección** del overlay: la contesta otro. `php artisan router:probe-overlay <id>` → estado `NO ES ÉL` (o `/ping <ip> count=2 ttl=1` en el CORE: si responde un tercero con `TTL exceeded`, es esto) | Se arregla **en el router cliente**, no en ISPWatch: `/interface l2tp-client print detail` (¿running?), `/ip address print` (¿está la `<ip>/32` dinámica sobre esa interfaz?). Un `disable`+`enable` de la interfaz la vuelve a negociar. Verifica también que no haya un segundo equipo discando con las mismas credenciales |
...
| **`/up` responde 200 y aun así nada funciona** | Es el chequeo por defecto de Laravel: no toca ninguna dependencia. Por diseño | Usa `/health`, que sí las verifica. Ver `ARQUITECTURA.md` § 16.2 |
| **`/health` devuelve el HTML del panel** | El catch-all del SPA está atendiendo la ruta: las de `routes/health.php` se registran después | El `where` del catch-all en `routes/web.php` debe excluir `health/` |
| **`checks.scheduler.status = fail`** | El componente `scheduler` no está desplegado, o murió. Nada del ciclo automático está corriendo | Desplegarlo (está en `.do/deploy.template.yaml`). Verifica con `php artisan system:heartbeat` seguido de una consulta a `/health` |
