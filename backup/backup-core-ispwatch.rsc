...
# IP Pública VPN MikroTik: 138.197.30.155
...
############################
# Solo crear perfil VPN si no existe
:if ([:len [/ppp profile find name=vpn-profile]] = 0) do={
  /ppp profile add name=vpn-profile local-address=10.10.10.1 remote-address=vpn-pool use-encryption=yes
}

/interface l2tp-server server
set enabled=yes use-ipsec=yes ipsec-secret="Q9!fZ@7M#rL2xS$A8DkE^pH%wC&y" authentication=mschap2 default-profile=vpn-profile

# Solo crear PPP secret si no existe
# Contraseña VPN generada: K7mP#9xR!vL2@qN8wT$jF5
:if ([:len [/ppp secret find name=core-isp-1]] = 0) do={
  /ppp secret add name=core-isp-1 password=K7mP#9xR!vL2@qN8wT$jF5 service=l2tp profile=vpn-profile
}
...
# 3. Permitir API desde servidor Laravel (162.159.140.98)
